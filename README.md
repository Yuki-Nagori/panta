...
| `cargo audit` | 通过根 runner 按固定版本准备 cargo-deny 到 `target/panta-tools/cargo`，执行 RustSec、许可证和依赖关系审计 |
...
| `cargo coverage` | 通过根 runner 按固定版本准备 cargo-llvm-cov 到 `target/panta-tools/cargo`，执行 Rust 覆盖率门禁；native 覆盖率由独立 CI job 负责 |
