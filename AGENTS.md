...
- 需要真实窗口验收时先运行 `cargo build --locked`，再直接启动 `target/native/debug/app/panta-native`。macOS 上 computer use 无法绑定裸二进制时，在忽略目录 `target/native/debug/app/PantaPreview.app` 创建临时 app wrapper：`Info.plist` 将 `CFBundleExecutable` 指向启动脚本，脚本切到仓库根目录并 `exec` 上述二进制；用 `cua.getApp` 绑定该 `.app`，再调用 `getAXStateAndScreenshot()` 检查真实窗口。每次 UI 操作后刷新 `getAXState()` 并按最新状态定位控件；验收结束关闭应用并删除临时 wrapper。不要为验收先连接 VS Code，也不要把无头 QML/CTest 或仅启动进程当作真实 GPU 窗口通过；Linux / Windows 使用窗口清单中的实际窗口 ID 绑定。图形会话不可用时记录具体限制，已有用户人工验收可单独记为用户证据。
