...
后续新任务使用当前最大编号加一，不复用已有编号。001–006 已完成；主线下一项先执行 [009 OCCT 依赖与 STEP 适配冒烟](task/009-occt-adapter-smoke.md)（CAE 接入），[007 VTK WebGPU 硬件窗口原生视口](task/007-vtk-quick-viewport.md) 的 SDK 前置已解除，进入原生叠加实现。技术规则见 [规范索引](standards/README.md)，产品目标见 [架构里程碑](architecture/milestones-and-validation.md)。
