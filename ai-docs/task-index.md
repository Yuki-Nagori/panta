...
| 009 | [OCCT 依赖与 STEP 适配冒烟](task/009-occt-adapter-smoke.md) | CAE 接入基础 | 003, 008 | in-progress |
...
后续新任务使用当前最大编号加一，不复用已有编号。001–006 已完成；主线当前项 [009 OCCT 依赖与 STEP 适配冒烟](task/009-occt-adapter-smoke.md)（CAE 接入）已实现并通过 macOS 全量验证，待 push 后三平台 CI 复验收尾；[007 VTK WebGPU 硬件窗口原生视口](task/007-vtk-quick-viewport.md) 已完成原生叠加实现且三平台 CI 全绿，剩真实窗口复验收尾。技术规则见 [规范索引](standards/README.md)，产品目标见 [架构里程碑](architecture/milestones-and-validation.md)。
