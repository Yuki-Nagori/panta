...
| Ninja | 1.13.2 | `github.com/ninja-build/ninja` release 资产（按平台选择） | SHA256 待 020 记录 | 预编译 release 资产；仅无可用资产时另立供给任务评估源码产物 | — | 可执行工具 | Apache-2.0（仓库 COPYING） | 003/004 |
...
| VTK | 9.7.0（commit `23f0a095621e`，当前最新稳定 tag） | `vtk.org` release/SDK 或项目制品源（具体平台包由 031 固定） | 预编译包 SHA256；源码 tag SHA 作为来源依据 | 预编译 SDK 优先；无匹配 SDK 时由独立制品任务评估生成，不在本地默认源码构建 | `GUISupportQtQuick`、Qt 6、目标图形后端和 ABI 由 007/031 核实 | `find_package(VTK CONFIG)` 与模块化 targets | BSD-3（发布包/源码树 Copyright.txt） | 007/031 |
| OCCT | 8.0.1（tag `V8.0.1`，commit `b8f597c67781`，2026-07-30，当前最新 release） | Open CASCADE 官方 SDK/包或项目制品源（具体平台包由 031 固定） | 预编译包 SHA256；tag SHA 作为来源依据 | 预编译 SDK 优先；无匹配 SDK 时由独立制品任务评估生成，不在本地默认源码构建 | 渲染/DRAW/Tcl-TK 相关关闭、模块裁剪由 009 定；SDK 必须提供 `find_package(OpenCASCADE)` | `find_package(OpenCASCADE CONFIG)` | LGPL-2.1 + OCCT 例外（发布包 LICENSE.txt） | 009/031 |
| Netgen | v6.2.2604（commit `3ee489c7d58f`，当前最新 release） | NGSolve 官方 release/SDK 或项目制品源（具体平台包由 031 固定） | 预编译包 SHA256；tag SHA 作为来源依据 | 预编译 SDK 优先；无匹配 SDK 时由独立制品任务评估生成，不在本地默认源码构建 | `USE_OCC=ON`、GUI/Python 关闭等由 010 定；SDK 与 OCCT ABI 必须匹配 | 由 010/031 核实 `find_package` targets | LGPL-2.1（发布包/仓库 LICENSE） | 010/031 |
...
- **VTK ↔ Qt**：VTK 9.7.0 的预编译 SDK 必须明确包含 `GUISupportQtQuick`/`QQuickVTKItem`、Qt 6.11.2 兼容范围和目标 ABI；007 验证实际窗口与运行时加载。没有匹配 SDK 时，先由 031 评估项目制品，不在开发机直接编译 VTK。
...
- Qt 6.11.2 Linux 预编译归档面向 RHEL9，Qt 工具的 `qtpaths`/`qmlimportscanner` 需要 ICU 73；Ubuntu CI 不伪造 ABI 或源码编译 ICU，而是跳过仅供 IDE 使用的 `.qmlls.build.ini` 和当前 app 的空 import scan，保留 QML typeinfo、cachegen、资源和运行时验证。
...
3. `cargo build --locked`：launcher 的 build.rs 调度 CMake/Ninja，消费构建树内固定的预编译依赖（缺失或 ABI 不匹配时立即失败并给出诊断）；`cargo run` 启动 `panta-native` 并转发参数与退出码。
