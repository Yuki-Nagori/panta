...
| VTK | 9.7.0（commit `23f0a095621e`，当前最新稳定 tag） | `vtk.org` release/SDK 或项目制品源（具体平台包由 031 固定） | 预编译包 SHA256；源码 tag SHA 作为来源依据 | 供给入口已落地（031：`native/cmake/sdk-provision.cmake` 的 `panta_require_sdk(vtk)`）；官方渠道无 C++ SDK，任何平台请求均按缺资产失败并指向 038，不源码构建 | `GUISupportQtQuick`、Qt 6、目标图形后端和 ABI 由 007/031 核实 | `find_package(VTK CONFIG)` 与模块化 targets | BSD-3（发布包/源码树 Copyright.txt） | 007/031 |
