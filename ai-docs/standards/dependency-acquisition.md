...
- 每个平台安装 rust-toolchain.toml 中的固定 Rust 工具链后执行 workspace check、完整 build（含测试目标编译）、`panta-tests toolchain` 实际路径核验；`cargo test --locked --workspace` 在独立 test job 消费 check job 传递的构建树运行，不重复编译。lint、format、audit 和两类 coverage 在 Ubuntu 独立运行，并按变更路径域触发。
