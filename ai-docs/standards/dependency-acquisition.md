...
| GoogleTest（仅测试） | v1.18.0（commit `063de7e9578f82b369302001269680b4b1553359`，当前最新 release） | `github.com/google/googletest` tag `v1.18.0` | 当前消费路径以完整 tag commit SHA 固定 FetchContent；任务 070 的手动 workflow 同一 SHA 构建三平台 SDK | 当前仍为 CMake FetchContent：仅 `BUILD_TESTING=ON` 拉取，`EXCLUDE_FROM_ALL`、`INSTALL_GTEST=OFF`；任务 070 生产的 SDK Release 待三平台产物发布并由后续 task 登记 URL/SHA256 后再接入消费 | `gtest_force_shared_crt=ON`；`BUILD_GMOCK=OFF` | `GTest::gtest` / `GTest::gtest_main` | BSD-3（SDK 随包 `share/licenses/GoogleTest/LICENSE`） | 019/070 |
