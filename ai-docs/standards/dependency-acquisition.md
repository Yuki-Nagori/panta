...
| LLVM/Clang | 22.1.7 | `github.com/llvm/llvm-project` release 资产：macOS ARM64 `LLVM-22.1.7-macOS-ARM64.tar.xz`（SHA256 `4177245188b0a30a6539c96b361dea56f253485756bfd8927a6a59e7301e7806`）、Linux x86_64 `LLVM-22.1.7-Linux-X64.tar.xz`（`edb0522b41e261819c06ea437d249f9b8acfa413d3805bc9920eec6fb76ff830`）、Windows x64 `clang+llvm-22.1.7-x86_64-pc-windows-msvc.tar.xz`（`3b568b5be1443d1a04c63261fa3a7aed16e126a8ed2196a1032aa8ed602144bd`） | Cargo build.rs 下载到 `target/panta-tools/llvm` 并校验；Unix 使用 clang/clang++，Windows 使用 clang-cl；clang-format/clang-tidy 与编译器来自同一目录 | — | Apache-2.0 with LLVM exceptions（发布包 LICENSE） | 042 |
...
托管工具归档下载以父进程限制含重试的总耗时为 30 分钟，解包上限为 20 分钟；超时终止并回收直接子进程，保留临时目录供下次持锁清理，不发布 `.complete`。安装锁、归档检查、摘要校验、解包和发布均有阶段日志；长时间运行的下载/解包每 30 秒报告耗时。Windows CI 的 check/build 使用 `-vv` 实时展示 build script 输出，失败或取消时尝试上传已有的 Cargo/CMake/CTest 诊断文件（运行器被强制终止时不保证上传）。
