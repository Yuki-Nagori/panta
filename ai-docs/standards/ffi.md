...
适用于 Rust↔C++，以及后续 Python bindings；不适用于外部求解器，后者继续走进程协议。任务 006 的 `panta-ffi` 已提供最小 CXX DTO、错误与 opaque 句柄（`rust::Box` 所有权）示例，不能据此推断业务服务边界已经完成。
...
- 桥接接口保持纯安全签名：DTO 用受支持的安全类型，错误经 `Result` 映射，调用方两侧都不写 unsafe。`unsafe extern` 声明与生成胶水内部的 unsafe 是 FFI 固有的信任边界与 ABI 成本，作为评审审查点保留，不作为清理目标。
