...
查阅 / 更新日期：2026-09-24。状态：Netgen 6.2.2604 制品消费、OCC 网格适配与调用侧销毁 workaround 已由任务 010 验收；commit `376f338` 的三平台 CI 全绿。SDK 修复进入消费基线后的 workaround 清理由任务 079 跟踪。
...
- 行为与坑（v6.2.2604 实测）：OCC 网格的名称表可能借用 `FaceDescriptor` 内部字符串；此外 ASan 确认 `Mesh::DeleteMesh()` 与随后 `Mesh::~Mesh()` 会重复释放 codim-1/2 名称。适配器在 `COMPAT(task 010; remove-task 079)` 中先从名称表摘除描述符别名，再调用 `DeleteMesh()`，清空两段清理间已经释放的 codim-1/2 槽位，最后析构 mesh；mesh 销毁后再释放 OCC 几何。该调用侧 workaround 不修改 SDK，已通过 macOS ASan/UBSan、LeakSanitizer、重复生成回归及 commit `376f338` 的三平台 CI；LSan 按仓库 suppression 清单抑制第三方栈帧，自有未抑制问题为零。此前将 mesh 保留到进程退出的临时处理已废弃；SDK 中确认修复并由项目消费后，按 [079](../task/079-remove-netgen-mesh-cleanup-workaround.md) 删除 workaround。Netgen 内部四面体方向与标准 FEM 正体积相反（box 实测 236/236 为负）且 `invert_tets` 在 OCC 路径未生效，转换层按有向体积符号归一化；`Ng_OCC_Load_STEP` 对不可解析输入抛 C++ 异常而非返回空，须在适配器边界捕获转结构化状态。
