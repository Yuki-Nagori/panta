...
`.pa` 是开发者直接编写的 UTF-8 文本字典/变量/主题源文件。首期语法采用 YAML 风格的冒号和两格缩进，但不实现通用 YAML；字典值不加引号，语言域直接以 English source 作为条目键，下面写 translation 和指令，变量/Theme 的类型表达式保留 `=`：
...
临时 TS 和 QM 不作为源码提交；失败时不覆盖上一份有效 TS/QM。编译器限制输入字节、token、嵌套深度和 message/变量数，拒绝未知必需版本、重复 source、非法 locale、占位符不一致和任意脚本/文件访问。

catalog 的 source-text 兼容索引按 source 长度降序构建，`ok ok` 优先于 `ok`，同长度冲突在校验阶段失败；QML/C++ 不走这个索引而使用完整 source 精确查找。
