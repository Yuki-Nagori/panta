...
以维护者提供的两张参考图为方向，统一 panta 桌面应用的视觉语言：浅色中性工作区、简洁的工具分组、辨识清楚且有节制的多色线性图标，以及具立体厚度和柔和投影的 Welcome `panta` 标志。当前 QML Theme 和图标体系沿用早期 HTML 截图底稿；新参考强调的颜色、图形和图标呈现需要先成为一套可维护的规则，再统一映射到 HTML 参考与实际 QML。
...
- 为 053 的 VTK Welcome 标志规定颜色、表面质感、厚度方向、背景/投影关系和留白约束；具体几何实现与真实窗口验收归 053。
