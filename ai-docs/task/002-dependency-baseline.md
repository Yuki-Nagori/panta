...
依赖 001 已完成。实施期间由维护者决策：依赖获取方式不采用系统包管理器，改为 Cargo 统一托管（见决策记录）；本机 Homebrew 安装的 Qt/VTK 等只作个人便利，不进入项目基线。
...
- 2026-09-16（实施）依赖获取方式（维护者决策）：**Cargo 统一托管**——开发者只需 git、rustup（工具链由 rust-toolchain.toml 固定）与 Apple 命令行工具；CMake/Ninja/Qt/VTK/OCCT/Netgen 由构建引导按固定清单拉取到构建树。否决系统包管理器路线：Homebrew 版本浮动、跨机器不可复现，且本机包不能作为团队基线。托管引导的实现（build.rs/超级构建细节）由 004 决定，本文只记录契约。
