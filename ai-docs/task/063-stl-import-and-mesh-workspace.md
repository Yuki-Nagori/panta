...
本任务先完成可验收的 HTML 状态参考和持久化边界设计，再分阶段实现 QML、C++ ViewModel、Rust 导入服务与视口数据流。HTML 参考必须覆盖新建项目弹窗这个之前遗漏的状态。
...
- 解析器版本 / 导入记录 ID，以及后续用于尺寸、拓扑和重建结果的稳定引用。
...
- 本阶段不实现 QML / C++ / Rust 导入代码，不修改已有 `.panta` schema，不直接操作 OCCT、Netgen 或 VTK。
- 不实现 Midplane、Dual Domain、Solid 3D 的实际网格生成，不把 Create Mesh 等任务伪装成已完成。
...
- [ ] 首期文件选择只接受 `.stl`，且格式集合可在不复制导入流程的情况下扩展。
- [ ] 取消 / 失败不修改工程；确认后 `.panta` 内有可重放 import record 和源资产引用。
- [ ] 重新打开工程可恢复导入记录、任务树和 STL 视口；版本 / 单位 / 尺寸错误可诊断。
- [ ] QML / C++ / Rust 分层、静态检查、native 测试、真实窗口与视口生命周期验证通过。

## 验证计划与结果

本阶段只交付文档和 HTML 参考：检查相对资源路径、无网络依赖、公共壳层复用、模板完整性和 `git diff --check`。浏览器交互仅用于演示文件选择、弹窗开关和页面跳转，不代表 QML / native 能力已实现。
...
已完成任务登记、`.panta` 持久化边界说明和首期 HTML 状态参考；STL 导入运行时实现及 native 验证待后续阶段。
