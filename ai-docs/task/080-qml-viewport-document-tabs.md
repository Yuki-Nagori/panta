...
- 状态：in-progress
- 阶段：应用平台扩展
- 依赖：[007](007-vtk-quick-viewport.md)、[063](063-stl-import-and-mesh-workspace.md)、[068](068-qml-project-and-layers-docks.md)、[073](073-flow-dsl-and-import-state-machine.md)
- 优先级：P1
- 负责人：Yuki
...
首次打开旧导入记录是本功能的异步领域操作：Rust 要按稳定导入 ID 解析工程资产并重建网格，操作可能受文件大小影响；工程切换、关闭待加载页签或快速连续选择还会产生取消和迟到结果。因此它作为 [073](073-flow-dsl-and-import-state-machine.md) 的首个 Flow 消费者。纯粹激活已就绪页签、改变选中态和关闭非活动页签仍是同步 UI 操作，不经 Flow。
...
- [QML 规范](../standards/qml.md)、[Flow 设计](../modules/flow-state-machines.md)、[Qt / Rust 边界](../architecture/native-domain-boundaries.md)
...
- 成功导入新 STL 后立即新增并激活对应页签，尽量复用导入事务已产出的网格快照，避免再次解析；点击工程树中已打开的记录只激活现有页签，未打开记录经 073 的异步 Flow 加载。
...
- 不把点击树项、页签选中、关闭按钮或 hover 状态写成 Rust Flow；Flow 只覆盖异步读取/解析与结果相关性。
...
- 073 冻结一个只读的工程资产激活契约：请求携带 project session / generation、导入记录 ID 与 attempt 相关性；Rust 校验工程记录并读取/解析已提交的 STL；返回拥有明确释放路径的 `SurfaceMesh` 快照；该路径不写清单、不推进修订。工程提交型导入 Flow 作为后续消费者另行登记。
...
- ViewModel 是活动文档 ID 与当前 viewport snapshot 的单一权威；QML 将其投影为选中页签并发送 open / activate / close 意图，不另行储存一个可能不同步的“当前 VTK 内容”。打开工程时 ViewModel 建立并激活 Welcome；页签选择与关闭属于 UI 选择状态，不使用 Rust Flow 或 Qt QStateMachine。
...
3. 先完成 073 的 Flow 元数据生成与只读 STL 工程资产激活消费者；Rust 返回经相关性校验的完成 / 失败 / 取消结果及可释放快照。
...
- `ai-docs/task/063-*`、`068-*`、`073-*` 和架构 / QML 文档：按最终行为同步现有“最新导入项”假设和 Flow 首个消费者说明。
...
| — | Rust Flow、Cargo 聚合、真窗口和 CPU / GPU 基准 | 按以上验收验证实际实现 | 未实施 |
...
- 2026-09-24：复核当前实现发现 Rust ProjectService 仅在内存持有最新网格，工程记录已有稳定 `ImportRecord.id` 与相对资产引用；首次激活其他已保存 STL 是真实读取/解析操作，拟作为 073 的首个异步 Flow 消费者。标签点击/关闭本身不进入 Flow。
- 2026-09-24：本任务只登记 HTML 原型和实现边界，不修改 QML 产品代码；打开标签的快照缓存上限需由真实 STL 基准决定。
- 2026-09-25：完成 HTML 参考的标签切换、关闭、固定宽度与水平拖动跟手和邻项动画；普通点击不捕获指针，真实拖动才由稳定列表捕获，拖动开始时激活目标标签。QML、ViewModel、Flow 和资源生命周期仍待实施。

## 完成摘要

未完成。HTML 交互参考已更新；QML、ViewModel、Flow、缓存/释放和性能验收仍待实施，任务保持 `in-progress`。
