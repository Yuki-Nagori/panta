...
维护者在本轮评审明确要求三平台统一 LLVM。已将 macOS/Linux/Windows 的自有 C++（CMake native 与 Cargo CXX 桥接）统一到固定 LLVM 22.1.7：macOS/Linux 使用 clang/clang++，Windows 使用同一发行包内的 clang-cl。当前 macOS 的 Apple Clang、Linux 的 GCC 和 Windows 的 cl.exe 将不再作为自有 C++ 默认编译器。
...
现存 `crates/launcher/build.rs`、`crates/panta-ffi/build.rs`、`.github/workflows/ci.yml`、`native/cmake/build-policy.cmake`、`crates/launcher/src/provision.rs`、tests 工具链核验、相关规范与任务索引。SDK workflow 仅在 038 范围确认需要重产时修改，不创建占位文件。

## 清理与兼容例外

已移除自有 C++ 对 Apple Clang/GCC/cl.exe 的默认选择；系统工具只在显式设置 `PANTA_USE_SYSTEM_TOOLS=1` 时使用，平台 SDK/运行库仍由宿主提供。无兼容例外，不为失败文件混用旧工具集；若验证失败，恢复该平台上一套完整可用配置。

## 验收标准

- [x] CMake 与 Cargo CXX 两条链显式使用同一 LLVM 22.1.7 目录；版本、来源、SHA256 可追溯，缺失明确失败；Rust 工具链不变。
- [x] macOS/Linux 选择 clang++，Windows 选择 clang-cl；Windows Visual Studio 生成器追加 `-T ClangCL`，Ninja 入口使用显式 clang-cl。
...
统一 LLVM 供给和编译器选择已落地：Cargo 默认下载并校验 LLVM 22.1.7 到 `target/panta-tools/llvm`，CMake 与 panta-ffi CXX 使用同一目录；macOS/Linux 选择 clang++，Windows 选择 clang-cl；clang-format 与 clang-tidy 复用该版本。系统工具仅通过 `PANTA_USE_SYSTEM_TOOLS=1` 显式旁路。待三平台 CI 冷构建、增量构建和 Windows VS/Ninja 实跑完成最终验收。
