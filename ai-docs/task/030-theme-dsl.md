...
将 Theme token 映射到自制 DSL 配置，通过独立应用主题服务与 C++ ViewModel 驱动 QML 组件切换。 当前仅规划，未实施。
...
先冻结键/类型/范围与 QML 属性映射，复用 025 无工程依赖的解析器；将默认值迁入内置 DSL 并让 Theme 绑定 ThemeViewModel；实现完整快照校验、GUI 线程发布、主题选择及偏好保存。
