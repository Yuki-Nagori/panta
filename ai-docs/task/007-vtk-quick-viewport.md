...
- 状态：blocked
- 阶段：M0
- 依赖：[005](005-qt-qml-shell.md)（已完成：Qt Quick 主窗口与预编译 Qt 6.11.2 就绪）、[031](031-prebuilt-native-dependencies.md)（WebGPU 硬件窗口制品待生产）
- 优先级：P0
- 负责人：待分配
- 创建 / 更新：2026-09-16 / 2026-09-20

## 阻塞（WebGPU 硬件窗口 SDK 待生产）

旧的 `QQuickVTKItem`/Qt OpenGL scenegraph 路线已经确认不适合作为当前实现：**窗口化渲染在 macOS 26（arm64，Qt 6.11.2 + OpenGLRhi）两种场景图循环下均崩溃**。该路线不再作为回退，也不保留兼容分支；待新的 WebGPU 硬件窗口 SDK 产出后，直接删除旧适配器、Qt VTK GUI 模块和失效引用。
...
解除条件：038/031 产出并登记新的 VTK 9.7.0 WebGPU 硬件窗口制品；所有平台至少包含 `VTK::RenderingWebGPU`、`VTK::RenderingUI` 和 `vtkWebGPURenderWindow`，并按平台提供 `vtkCocoaHardwareWindow`/`vtkCocoaHardwareView`、`vtkWaylandHardwareWindow`/`vtkWaylandRenderWindowInteractor` 或 `vtkWin32HardwareWindow`。Linux 当前制品明确为 `VTK_USE_X=OFF`、`VTK_USE_Wayland=ON`，纯 X11 环境不属于该资产的运行目标；若产品需要纯 X11，必须另产 X11 变体。随后在 007 内完成各平台原生 surface/view 与 Qt Quick 界面的叠加、尺寸同步和事件协调。诊断盲区：原生 SIGSEGV/SIGTRAP 在控制台零输出，历史证据只能取自 `~/Library/Logs/DiagnosticReports/*.ips`。
...
当前任务等待 038 生产、031 登记新的 VTK WebGPU 硬件窗口 SDK；实现前核对其 CMake targets、头文件和 ABI。不得在本地通过 FetchContent 或其他方式默认编译 VTK 源码。
...
依赖 005 已完成，031/038 正在切换到新的 VTK WebGPU 硬件窗口制品。实施前核对预编译 VTK 9.7.0 SDK 的实际头文件、CMake targets、Apple/Windows/Linux ABI 和图形后端；Linux 先按 Wayland 目标验证，纯 X11 另行登记变体；缺少匹配包时保持 blocked，不启动本地源码构建。模块 URI 的正式自动注册仍由 026 负责，007 先沿用 005 的显式注册边界。
...
| 2026-09-20 | WebGPU 硬件窗口路线与 SDK 生产契约核对 | 旧 `sdk-vtk-9.7.0` 只满足 `GUISupportQtQuick`/`RenderingQt`；新制品配置改为 `VTK_ENABLE_WEBGPU=ON`、`RenderingWebGPU` + `RenderingUI`，平台窗口为 Linux Wayland、macOS Cocoa、Windows Win32，发布 tag 为 `sdk-vtk-9.7.0-webgpu`，待维护者 dispatch 后补 URL/SHA256 与实际头文件验证 |
...
- 2026-09-19（路线切换）：放弃 Qt OpenGL/QQuickVTKItem 场景图集成，改为 VTK WebGPU render window + 平台 hardware window；macOS 使用 `vtkCocoaHardwareWindow`/`vtkCocoaHardwareView` 通过原生 Metal surface 与 Qt Quick 界面叠加，Linux 使用 Wayland，Windows 使用 Win32；038 先生产 `sdk-vtk-9.7.0-webgpu`，031 在取得 URL/SHA256 后再切换消费 manifest。
