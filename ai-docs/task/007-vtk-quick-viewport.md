...
| 2026-09-20 | CI run 35510679878 复验（85bff0c，全绿）：Windows Test **27/27**（Qml.ShellModuleLoads 0.10s、Qml.ViewportModuleLoads 0.05s Passed），整轮 4m1s（历史最长 53m38s）；取证步按修复后环境直接拉起注册 exe 退出码 0，QTest init/loads/cleanup 全 PASS——根因结论与 SDK bin PATH 修复双双验证 | Windows 挂起结案，三平台测试恢复绿色。遗留记录：产品 app（panta-native.exe）同样导入 VTK DLL，窗口化运行需要同样的 PATH 策略或安装期 DLL 复制（归入 031/007 后续）；CI 取证步保留作早期诊断（健康时亚秒级、非致命），稳定数轮后可简化 |
