...
| 2026-09-20 | CI run 35510679878 复验（85bff0c，全绿）：Windows Test **27/27**（Qml.ShellModuleLoads 0.10s、Qml.ViewportModuleLoads 0.05s Passed），整轮 4m1s（历史最长 53m38s）；取证步按修复后环境直接拉起注册 exe 退出码 0，QTest init/loads/cleanup 全 PASS——根因结论与 SDK bin PATH 修复双双验证 | Windows 挂起结案，三平台测试恢复绿色。收尾简化：取证步移除（使命完成），Defender 豁免保留作构建加速；`native_test_env` 的 PATH 合并收拢为 retain+push。遗留记录：产品 app（panta-native.exe）同样导入 VTK DLL，窗口化运行需要同样的 PATH 策略或安装期 DLL 复制（归入 031/007 后续） |
| 2026-09-20 | Defender 豁免步骤去留评估：对比加步骤前后各轮 Windows Check+Build 步耗时（129s 基线 vs 81–129s），热缓存增量构建下无可测差异——扫描开销只在全量冷构建（LLVM 2GB 解包 + 全量 Ninja 写盘）场景显著，属低频事件 | 步骤移除（维护者确认简化）：原始动机已证伪、热构建无收益、少一个潜在失败点；冷构建若实测明显变慢，一行加回并配 continue-on-error。任务 031 同步记录 Windows VTK SDK 为 DLL 形态的运行时分发约束 |
