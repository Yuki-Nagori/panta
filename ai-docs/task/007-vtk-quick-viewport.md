...
| 2026-09-20 | CI run 35508804535 取证（轮 4）：Defender 豁免未解除挂起（Qml.ShellModuleLoads 仍 120s 超时）——Defender 理论证伪；取证步修复 exe 路径后首次产出关键数据：Windows 上 exe 位于构建根（非 `app/` 子目录），直接拉起 **60ms 内静默退出**（stdout/stderr 零字节，退出码因 Start-Process 缺省 ShellExecute 拿不到句柄未捕获），而 ctest 拉起同一 exe 却永久挂死——同一 exe 两种结局，差异聚焦 stdio 形态（文件/控制台 vs 管道）与进程句柄获取方式 | Defender 豁免保留但改注释为构建加速用途；取证步升级为决定性矩阵：llvm-readobj 导出 PE 导入表（核 import DLL 清单）、控制台 stdio 与管道 stdio（.NET Process 真实句柄 + 30s 有界 + 杀树后回读管道部分输出）分别拉起、WER 事件按 ProviderName 过滤（上轮 Id 过滤只命中 WMI 噪音）。行为待下轮 CI 复验 |
