...
- 2026-09-21（增量五，任务 049，发布语义与 ISA 基线）：维护者决策重产即**单一 tag 全量覆盖**——两条管线新增收口 publish job，等三平台制品齐备后清空 Release 全部旧资产再重传（矩阵并发删/传有竞态，收敛到单 job；改名/停产的旧文件不再残留，取代手动删资产）。`netgen.cmake` 显式 `USE_NATIVE_ARCH=OFF`：上游默认 ON 使 Linux 制品携带 `-march=native`（v6.2.2604 首版 linux 制品内含 AVX-512，无 AVX-512 消费机加载即 SIGILL，任务 049 根因），预编译 SDK 一律 generic 基线出包。重产覆盖后按实测 SHA256 同步 031 manifest。
