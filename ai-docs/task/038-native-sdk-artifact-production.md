...
| 2026-09-18 | 制品自检与打包：selfcheck `OpenCASCADE`（`TKernel TKDESTEP`，OCCT 8 targets 无命名空间）、`netgen`（`ngcore nglib`，无命名空间）；package 平铺归档 | 双双通过；LGPL-2.1+例外（OCCT）/LICENSE（Netgen）随包，`panta-sdk.json` 齐。Netgen 的 `libnglib.dylib` 链接同管线 OCCT（同工具链，ABI 约束满足）；运行期 rpath 跨目录解析留待 009/010 消费侧处理（已记录）。三 workflow（sdk-vtk/occt/netgen.yml）YAML 解析通过，独立 dispatch、互不牵连（维护者决策：VTK 已发布，不再与 OCCT/Netgen 绑定同管线） |
