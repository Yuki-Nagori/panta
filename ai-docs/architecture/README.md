...
本组文档依据用户提供的架构讨论附件整理。附件将桌面平台称为 MoldCAE / `moldcae`；本仓库名称是 `chronos`，因此以下以 chronos 指代该桌面平台。MoldCAE 是原方案名称，不代表已存在的本地目录或可执行文件。

当前仓库没有应用源码、构建脚本、依赖锁文件和测试。本组文档描述目标架构与建议契约，不宣称功能已实现；具体依赖版本、协议编码和持久化格式仍须在实施时验证并确定。文档中的字段名和 API 名用于说明语义，不是已发布接口。
