...
计划需要支持 edition 2024 的 Rust stable 工具链、支持 C++20 的编译器、CMake、Ninja，以及 Qt 6、VTK、OCCT、Netgen 的开发依赖。Python 3.12+ 等到自动化模块实施时引入。

首次集成时记录：主验证操作系统与架构、编译器版本、C++ 标准库、构建配置、Qt 图形后端、依赖来源和精确版本。第三方依赖不必自身以 C++20 编写，但头文件、ABI、运行库和链接方式必须与应用兼容。当前未承诺跨平台矩阵，也没有指定包管理器或安装命令。
