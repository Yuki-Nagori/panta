...
Qt Quick 主窗口已可用（任务 005）：`native/app` 为 Qt 入口，`native/bridge` 提供 ViewModel（GTest 信号测试），`qml/`（URI `Panta.Shell`，NO_PLUGIN 资源模块）承载界面；Qt 6.11.2 预编译包由 `native/cmake/qt-provision.cmake` 按三平台固定清单下载到构建树。任务 006 的 `panta-ffi` 以 CXX 1.0.202 生成 Rust/C++ 桥接静态库和 native 边界测试，launcher build.rs 将生成头与静态库路径传入 CMake；该最小路径已由三平台 CI 干净构建覆盖。QML/资源目录已纳入 build.rs 重建追踪（改 QML 即重跑 qmlcachegen）。此时尚未接入 VTK/OCCT/Netgen，界面为骨架占位，不宣称桌面功能。
